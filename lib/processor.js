...
// clamscan.init() is async so we hold a Promise and await it each time.
// After the first successful init the same resolved scanner is reused.
let _scannerPromise = null;

function getScanner() {
  if (!_scannerPromise) {
    _scannerPromise = new ClamScan().init({
      preference: 'clamdscan',
      clamdscan: {
        // Ubuntu/Debian clamd listens on a Unix socket by default, not TCP.
        // Set PRATIMA_CLAMAV_HOST + PRATIMA_CLAMAV_PORT to switch to TCP mode.
      ...
      // Reset so the next call retries instead of reusing a failed Promise
 ...
 * Streams the buffer directly over the Unix socket — clamd never touches
 * the filesystem, so there are no file-permission issues.
 * Returns true  = clean.
 * Returns false = infected, or scanner unavailable (fail-closed by default).
 * Set PRATIMA_CLAMAV_OPTIONAL=1 in .env to fail-open when ClamAV is down.
 */
async function scanBuffer(buffer) {
  try {
    ...
    // isInfected: false = clean, true = infected, null = scan error
