...
module.exports = (program) => {
  ...
  program
    ...
    .option('--origin <urls>', 'Comma or space-separated allowed frontend origins')
    .action((name, opts) => {
      try {
  ...
  program
    ...
    .action((name, urls) => {
      if (!urls || urls.length === 0) {
        ...
        console.error('     Example: pratima set-origin mysite https://mysite.com http://localhost:5173');
        process.exit(1);
      }
      for (const u of urls) {
        try { new URL(u); } catch {
  ...
  program
    ...
    .action((name) => {
      ...
      if (!token) {
        console.error(`  ✗  No token set for "${name}" — try: pratima token-regen ${name}`);
        process.exit(1);
      }
      console.log('');
  ...
  program
    ...
    .action((tenantName) => {
      ...
      console.log(JSON.stringify({ ...stats, allowedOrigins: origins.length ? origins : [] }, null, 2));
    });
