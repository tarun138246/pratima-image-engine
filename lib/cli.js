...
module.exports = (program) => {
    ...
    .action((name, opts) => {
      try {
        ...
        if (opts.origin) {
          ...
          console.log(`  Allowed origin set: ${opts.origin}`);
        } else {
          console.log(`  Next step — set the allowed frontend URL:`);
          console.log(`    pratima tenant set-origin ${name} https://your-frontend.com`);
        }
        console.log('');
      } catch (err) {
  ...
  // ── tenant limits set ────────────────────────────────────────────────────
  program
    .command('tenant limits set <tenant>')
  ...
  // ── tenant set-origin ────────────────────────────────────────────────────
  program
    .command('tenant set-origin <name> <url>')
    .description('Set or update the allowed frontend URL for a tenant (e.g. https://mysite.com)')
    .action((name, url) => {
      try {
        // Basic URL validation
        new URL(url);
        tenant.setOrigin(name, url);
        console.log(`  ✓  Allowed origin for "${name}" → ${url}`);
      } catch (err) {
        console.error(`  ✗  ${err.message}`);
        process.exit(1);
      }
  ...
  program
    ...
    .action((tenantName) => {
      ...
      const origin = tenant.getOrigin(tenantName) || '(not set)';
      console.log(JSON.stringify({ ...stats, allowedOrigin: origin }, null, 2));
    });
