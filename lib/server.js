...
const ALLOWED_TEXT_FIELDS = new Set(['alt', 'title']);
...
//   tags    — comma-separated tag list (e.g. "product,red,summer")
//   folder  — logical subfolder (e.g. "products/shoes")
//
// Response 202:
...
app.post('/upload/:tenant',
  ...
  async (req, res, next) => {
    ...
    const imageId = processor.generateImageId(tenantName);
...
app.get('/image/:tenant/:id',
  auth.requireValidTenantName,
  (req, res, next) => {
    if (!/^pratima-[a-z0-9]+-\d+-[A-F0-9]+$/i.test(req.params.id)) {
