...
// Validate origin/referer against one or more comma-separated allowed origins
function originMatches(req, allowedOrigins) {
  const list    = allowedOrigins.split(',').map(o => o.trim()).filter(Boolean);
  const origin  = (req.headers['origin']  || '').trim();
  const referer = (req.headers['referer'] || '').trim();
  return list.some(o => origin.startsWith(o) || referer.startsWith(o));
}
...
function requireAllowedOriginOrKey(req, res, next) {
  ...
  const allowedOrigin = db.getTenantOrigin(tenantName);
  if (!allowedOrigin) {
    return res.status(403).json({ error: 'Tenant has no allowed origin configured — set one with: pratima set-origin <tenant> <url>' });
  }
