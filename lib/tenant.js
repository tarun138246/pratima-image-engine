...
/**
 * Set one or more allowed origins for a tenant.
 * Accepts an array of URLs, or a single comma/space-separated string.
 * Stored internally as a comma-separated list.
 *
 * Examples:
 *   setOrigin('shop', ['https://shop.com', 'http://localhost:5173'])
 *   setOrigin('shop', 'https://shop.com,http://localhost:5173')
 */
function setOrigin(name, origins) {
  if (!exists(name)) throw new Error(`Tenant "${name}" does not exist`);

  const raw = Array.isArray(origins) ? origins.join(',') : String(origins);
  ...
  if (!normalized) throw new Error('No valid origins provided');
  db.setTenantOrigin(name, normalized);
  logger.info('tenant origins updated', { tenant: name, origins: normalized });
}
